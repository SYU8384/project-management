...
- **Indexing:** do not maintain a complete note register here. Per-note frontmatter is the source of processing state.`,
  ...
  writeCreateOnly(join(pmFolder, "roadmap/plans/plans.md"), folderNote({
    title: "plans",
    intro: "Concrete plans, implementation strategies, and design approaches. Active plans are mirrored in `roadmap/done-pending.md` and link back to their exact mirror from `## Related`; implemented plans move to `archive/` after the user verifies the work and approves archival. Significant decisions live in `decisions/` and are cited from the plan, not duplicated here.",
...
- **Cross-link:** when a planning note is approved, add a slug-only \`## <slug>\` section to \`roadmap/done-pending.md\` with the date-prefixed planning note link and required human archive-confirmation checkbox. Active/proposed planning notes also keep \`## Related\` linked back to \`Done-pending mirror: [[${linkRoot}/roadmap/done-pending#<slug>|done-pending#<slug>]]\` and repeat relevant decision/feature/system/docs links already present in that mirror. When it ships and the user approves archival, mark the confirmation checkbox done, distill durable current truth into \`system/\`, and archive the file.
