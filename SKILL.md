...
**2. Set up project management** — say "setup", "setup this repo", "setup PM", or "setup as collaborator". The agent first inspects the current folder and `~/.config/project-management/projects.json`; if the folder is empty or unrecognized, it asks whether the folder is the code repo, PM folder, or something else. Then it asks a short guided intake with selectable answer suggestions (role, PM folder state, project phase, AGENTS.md setup) plus free-text path/description questions only when needed. It then routes to create a new PM folder, repair an existing/messy PM folder, or register read-only collaborator access. (Contributors with no PM access at all don't register projects — they submit PRs with PM folder impact notes, and the maintainer applies PM updates on merge.) For owner setup with an empty code repo and empty PM folder, run `scripts/bootstrap-pm.mjs` to scaffold the PM folder and create the authoritative code repo `AGENTS.md` when a real `code_repo` path exists. v1.3.0+ stores `projects.json` at `~/.config/project-management/projects.json`, not in the skill directory.
...
**5. Add an AGENTS.md PM folder section to a project repo** — say "add to AGENTS.md" or "set up AGENTS.md for <project>". The agent reads the project's `access` field in `~/.config/project-management/projects.json` and copies the matching template into the project's `AGENTS.md`: `AGENTS_PM_SECTION_AUTHORITATIVE.md` for `access: authoritative`, or `AGENTS_PM_SECTION_READONLY.md` for `access: read-only`. There is no third template; contributors with no PM access at all are not users of the skill on their side.
...
| "summarize this project", "summarize <ProjectName>", "what's going on with this project" | Summarize: read `README.md`, `CURRENT_STATUS.md`, `PRODUCT.md`, and roadmap's `Active` items; produce a 1-paragraph overview | Summarize This Project (REFERENCE) |
